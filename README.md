...
Если точного FAQ нет, бот даёт осторожный приблизительный ответ на основе общих знаний модели, явно отмечает отсутствие подтверждения в справке и предлагает написать на `info@x-raydent.ru`. Режим можно отключить через `XRAYDENT_ALLOW_GENERAL_KNOWLEDGE=false`. Медицинские вопросы, острые симптомы, prompt injection и чувствительные данные по-прежнему обрабатываются safety-router без свободной генерации.
